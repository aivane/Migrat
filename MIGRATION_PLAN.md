...
5. **[fundinfoApi.js](src/services/fundinfoApi.js)** — คอลัมน์ "สิทธิภาษี" โชว์ "-" ตลอด ทั้งที่ API มี field `fund_tax_type`/`tax_saving_fund` จริง (ตรวจข้อมูลจริง 2,000 กองทุนพบค่า RMF/SSF/TESG/TESGX ใช้งานได้จริง 259 กอง) แค่ไม่เคยถูก map เข้า `normalizeFund()` เพิ่ม `taxBenefit` ให้ตรงกับ `TAX_BENEFIT_OPTIONS` ที่มีอยู่แล้ว ทดสอบแล้วเห็น "RMF" จริงในตาราง (14/37 กองทุนไทยหน้าแรก)
6. **[useFundAnalytics.js](src/composables/useFundAnalytics.js)** — แท็บ "ค่าธรรมเนียม" ช่อง Front-end/Back-end Fee (เก็บจริง) โชว์ "-" ตลอด ทั้งที่ API มี field `front_end_fee`/`back_end_fee` จริง (มีข้อมูลจริง 618/521 กองทุนจาก 2,000 กองที่ตรวจ) และถูก capture ไว้ใน `fund.frontEndFee`/`fund.backEndFee` อยู่แล้วโดย `normalizeFund()` แค่ `feeSchedule` ไม่เคยเอามาใช้ ทดสอบแล้วเห็น "3.21 %" จริงสำหรับ SCBSEMI(A)
7. **[useFundAnalytics.js](src/composables/useFundAnalytics.js) + [FundOverviewPanel.vue](src/components/fundinfo/detail/FundOverviewPanel.vue)** — แท็บ "กราฟภาพรวม" ว่างเปล่าตลอดในโหมด API (ไม่มีราคา/NAV ย้อนหลังรายวันจริงๆ) แก้โดยสร้างกราฟจาก checkpoint ผลตอบแทนจริงที่มีอยู่แล้ว (`return_1m/3m/1y/3y/5y/10y`) แปลงเป็น index ฐาน 100 ย้อนหลัง — ไม่มีจุดไหนมโนขึ้นมาเอง ทุกจุดมาจากผลตอบแทนจริงที่ API เปิดเผย ปรับ default โหมดเป็น "ผลตอบแทน (%)" (แทน "ราคา NAV" เพราะไม่มีราคาจริงให้ใช้) พร้อม footnote บอกตรงๆ ว่าไม่ใช่ราคาปิดรายวัน ทดสอบแล้วเห็นกราฟขึ้นจริง
8. **[fundinfoApi.js](src/services/fundinfoApi.js)** — เพิ่ม field `retPRaw` (เหมือน `retP` แต่ null เมื่อไม่มีข้อมูลจริง แทนที่จะ default เป็น 0%) เพื่อแก้บั๊กแฝงในข้อ 7: ของเดิมถ้ากองทุนอายุ <10 ปีจะโชว์ "ผลตอบแทน 10 ปีก่อน = 0%" ราวกับเป็นข้อมูลจริง ทั้งที่จริงคือไม่มีข้อมูลช่วงนั้นเลย แก้ให้กราฟภาพรวม (ข้อ 7) ใช้ `retPRaw` แทน
9. **[useFundinfoInsight.js](src/composables/useFundinfoInsight.js) + [useFundinfoRanking.js](src/composables/useFundinfoRanking.js)** — กราฟ "เปรียบเทียบผลตอบแทนบนกราฟเดียวกัน" (Insight Compare, section ③) ว่างเปล่าตลอดในโหมด API เพราะ `series` ถูก hardcode เป็น `null` เสมอ ทั้งที่ entity kind `holder` (กองทุนไทยที่ถือหุ้นต่างประเทศ) และ `master` (กลุ่ม Master Fund ของ Feeder) มี checkpoint ผลตอบแทนจริงอยู่แล้ว (`retPRaw` ต่อกองทุน / ค่าเฉลี่ย null-aware ต่อกลุ่ม master ที่เพิ่มใน `buildMasterFundEntities`) แก้ให้สร้างกราฟจาก checkpoint จริงเหล่านี้ (ลากเส้นตรงเชื่อมจุดจริง ไม่ใช่ราคาปิดรายวัน มี footnote บอกตรงๆ) ส่วน entity kind `stock` (หุ้นรายตัวจาก `/stocks/top`) ยังคงไม่มีกราฟ เพราะ endpoint นั้นไม่มีผลตอบแทนให้เลยจริงๆ ทดสอบแล้วเห็นกราฟขึ้นจริงทั้งหน้า Feeder (master) และ Offshore (holder) โดยที่ entity แบบ stock ยังคงถูกตัดออกจากกราฟอย่างถูกต้อง (ไม่ error ไม่มโนเส้น)
...
### ปัญหาคุณภาพข้อมูลจาก backend (ตั้งใจไม่แก้ที่ frontend — ตามคำตัดสินใจของทีม 2026-08-25)

- **การ์ด "เงินไหลเข้าสูงสุด" โชว์ตัวเลขผิดปกติ (+B187,848.31 พันล.)** — ตรวจ raw data แล้วพบสาเหตุจาก **กองทุนเดียว `KKP TECH-H-F`** ที่ API รายงาน `estimated_flow_1m_m_thb = 187,840,771.72` (ล้านบาท) และ `unit_change_1m = 217,219,466%` ทั้งที่ AUM ของกองนี้มีแค่ 86.48 ล้านบาท — เป็น data anomaly จากต้นทาง (backend) ไม่ใช่บั๊กการคำนวณของ frontend (frontend รวมยอดถูกต้องแล้ว) ทีมตัดสินใจ**ปล่อยไว้ตามที่ API ส่งมา ไม่แก้ที่ frontend** เพื่อให้เห็นปัญหานี้ผ่านหน้าเว็บและไปแจ้งทีม backend ตรวจ record ของ `KKP TECH-H-F` แทน

### เหลือ (ติดที่ API ไม่มีข้อมูลจริง ไม่ใช่บั๊กโค้ด — ยืนยันด้วยการยิง API จริงแล้ว ไม่ใช่แค่ไม่ได้ map)

- **Calendar Year Returns ในหน้า Detail** — ไม่มี field ผลตอบแทนรายปีปฏิทินเลยสักตัวในระบบ (ไม่เหมือนกราฟภาพรวม/Insight Compare ที่พอมี checkpoint ผลตอบแทนจริงให้ประกอบเป็นกราฟได้ อันนี้ไม่มีข้อมูลดิบให้ใช้เลย) — ต้องรอ backend เพิ่ม field ประเภทนี้ก่อน
- **ราคาปิดรายวันจริง (สำหรับกราฟภาพรวมและ Insight Compare)** — ยังไม่มี แก้ได้แค่ระดับ "ลากเส้นตรงเชื่อม checkpoint จริง" (ดูข้อ 7-9 ด้านบน) ยังไม่ใช่กราฟราคาปิดรายวันจริง ต้องรอ backend เพิ่ม endpoint ราคาย้อนหลัง (ลองยิง `/api/v1/funds/{code}/trend` ดูจริงแล้วพบว่าเป็นแค่ "holdings trend" ไม่ใช่ราคา) ก่อนถึงจะทำกราฟแบบละเอียดรายวันได้
- **กราฟ Insight Compare สำหรับ entity แบบ `stock`** (หุ้นรายตัวจาก `/stocks/top`) — ยังไม่มีกราฟ เพราะ endpoint นั้นไม่มีผลตอบแทนของหุ้นให้เลย (มีแค่ fundCount/holding weight) ต่างจาก `holder`/`master` ที่แก้ได้แล้ว (ข้อ 9)
- **Alpha / Beta / Recovering Period** — ไม่มี field เหล่านี้ใน `/funds/list` หรือ `/funds/{code}` เลย
- **เฉลี่ยกลุ่ม (group average return)** ในตารางเปรียบเทียบผลตอบแทน — ไม่มี peer-group average ให้จาก API
- **Turnover ratio, FX Hedging %** — ไม่มี field เหล่านี้ใน API
- **"ขั้นต่ำ" (มูลค่าซื้อขั้นต่ำ) และ "NAV/หน่วย" ในตารางกองทุน** — ยืนยันแล้วว่า API ไม่มี field พวกนี้เลยทั้งใน `/funds/list` และ `/funds/{code}`
- **Switching-in / Switching-out Fee** (prospectus และ actual ทั้งคู่) และ **Front-end/Back-end Fee ฝั่ง "ตามหนังสือชี้ชวน"** — API มีแค่ตัวเลข "เก็บจริง" ปัจจุบัน (front_end_fee/back_end_fee) ไม่มี prospectus rate แยกและไม่มี switching fee เลย
